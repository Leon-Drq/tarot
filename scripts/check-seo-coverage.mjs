...
assertIncludes(files.dailyTarotPage, "Start free reading", "Daily Tarot direct free reading CTA copy")
...
const identityMetadataCoverage = [
  ...
  [files.trustPages, "96 x 96 search favicon", "brand asset favicon copy"],
...
const dailyReminderCoverage = [
  ...
  [files.reminderPrivateGrantMigration, "revoke execute on function private.mark_daily_tarot_reminder_failed(text, uuid, text) from anon, authenticated", "private reminder failed direct revoke"],
  [files.reminderPrivateGrantMigration, "revoke execute on function private.disable_daily_tarot_reminders(text, uuid) from anon, authenticated", "private reminder unsubscribe direct revoke"],
  [files.reminderPrivateGrantMigration, "revoke usage on schema private from anon, authenticated", "private schema usage revoke"],
  [files.reminderPrivateGrantMigration, "grant execute on function public.daily_tarot_reminder_candidates(text, integer) to anon, authenticated", "public reminder candidates wrapper grant"],
  [files.reminderPrivateGrantMigration, "grant execute on function public.disable_daily_tarot_reminders(text, uuid) to anon, authenticated", "public reminder unsubscribe wrapper grant"],
